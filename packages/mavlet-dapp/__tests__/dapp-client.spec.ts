...
import { DAppClient, MavletEvent } from '../src'
...
describe(`DAppClient`, () => {
  ...
  it(`should read active account from storage during initialization`, async () => {
    ...
    expect(storageStub.firstCall.args[0]).to.equal(StorageKey.MAVLET_SDK_SECRET_SEED)
  ...
  it(`should have a mavlet ID`, async () => {
    const dAppClient = new DAppClient({ name: 'Test', storage: new LocalStorage() })
    expect(typeof (await dAppClient.mavletId)).to.equal('string')
  })
  ...
  it(`should get app metadata`, async () => {
    ...
    expect(metadata).to.deep.equal({
      senderId: await getSenderId(await dAppClient.mavletId),
  ...
  it(`should subscribe to an event`, async () => {
    ...
    const cb = () => {}
    dAppClient.subscribeToEvent(MavletEvent.ACTIVE_ACCOUNT_SET, cb)

    expect(eventsStub.callCount).to.equal(1)
    expect(eventsStub.firstCall.args[0]).to.equal(MavletEvent.ACTIVE_ACCOUNT_SET)
  ...
  it(`should throw an error when checking for permissions and no active account is set`, async () => {
    ...
    try {
      await dAppClient.checkPermissions(MavletMessageType.OperationRequest)
      throw new Error('Should have failed')
    } catch (e) {
      expect(eventsStub.firstCall.args[0]).to.equal(MavletEvent.ACTIVE_TRANSPORT_SET) // Called in the constructor
      expect(eventsStub.firstCall.args[1]).to.equal(undefined)
      expect(eventsStub.secondCall.args[0]).to.equal(MavletEvent.INTERNAL_ERROR)
      expect(eventsStub.secondCall.args[1]).to.deep.equal({ text: 'No active account set!' })
      expect(eventsStub.thirdCall.args[0]).to.equal(MavletEvent.ACTIVE_ACCOUNT_SET) // Called in the constructor
  ...
  it(`should check permissions for a PermissionRequest`, async () => {
    ...
    const hasPermission = await dAppClient.checkPermissions(MavletMessageType.PermissionRequest)
  ...
  it(`should check permissions for an OperationRequest`, async () => {
    ...
    expect(await dAppClient.checkPermissions(MavletMessageType.OperationRequest)).to.be.true

    getActiveAccountStub.resolves({
      scopes: [PermissionScope.SIGN]
    } as any)

    expect(await dAppClient.checkPermissions(MavletMessageType.OperationRequest)).to.be.false
  })
  ...
  it(`should check permissions for a SignPayloadRequest`, async () => {
    ...
    expect(await dAppClient.checkPermissions(MavletMessageType.SignPayloadRequest)).to.be.true

    getActiveAccountStub.resolves({
      scopes: [PermissionScope.OPERATION_REQUEST]
    } as any)

    expect(await dAppClient.checkPermissions(MavletMessageType.SignPayloadRequest)).to.be.false
  })
  ...
  it(`should check permissions for a BroadcastRequest`, async () => {
    ...
    expect(await dAppClient.checkPermissions(MavletMessageType.BroadcastRequest)).to.be.true
  })
  ...
  it(`should send an internal error`, async () => {
    ...
    try {
      ...
      expect(eventsStub.firstCall.args[0]).to.equal(MavletEvent.INTERNAL_ERROR)
      expect(eventsStub.firstCall.args[1]).to.deep.equal({ text: 'some-message' })
      expect(eventsStub.secondCall.args[0]).to.equal(MavletEvent.ACTIVE_TRANSPORT_SET)
  ...
  it(`should handle request errors (case: mavlet error)`, async () => {
    const dAppClient = new DAppClient({ name: 'Test', storage: new LocalStorage() })
    ...
    const error = {
      errorType: MavletErrorType.NOT_GRANTED_ERROR
    }
    ...
    try {
      ...
      expect(eventsStub.getCall(0).args[0]).to.equal(MavletEvent.ACTIVE_TRANSPORT_SET)
      expect(eventsStub.getCall(0).args[1]).to.equal(undefined)
      expect(eventsStub.getCall(1).args[0]).to.equal(MavletEvent.ACTIVE_ACCOUNT_SET)
      expect(eventsStub.getCall(1).args[1]).to.equal(undefined)
      expect(eventsStub.getCall(2).args[0]).to.equal(MavletEvent.ACTIVE_TRANSPORT_SET)
      expect(eventsStub.getCall(2).args[1]).to.equal(undefined)
      expect(eventsStub.getCall(3).args[0]).to.equal(MavletEvent.PERMISSION_REQUEST_ERROR)
  ...
  it(`should handle request errors (case: not mavlet error)`, async () => {
    const dAppClient = new DAppClient({ name: 'Test', storage: new LocalStorage() })
  ...
  it(`should send notifications on success`, async () => {
    ...
    const request = { type: MavletMessageType.PermissionRequest }
    const response = {}

    await (<any>dAppClient).notifySuccess(request, response)
    expect(eventsStub.callCount).to.equal(1)
    expect(eventsStub.firstCall.args[0]).to.equal(MavletEvent.PERMISSION_REQUEST_SUCCESS)
  ...
  it(`should store an open request and handle an error response`, async () => {
    ...
    const message = { type: MavletMessageType.Error, id: 'my-id' }
    const connectionInfo = {}
  ...
  it(`should handle a disconnect message`, async () => {
    ...
    const message = { type: MavletMessageType.Disconnect, id: 'my-id', senderId: 'sender-id' }
    const connectionInfo = {}
    ...
    expect(eventsStub.getCall(0).args[0], '1').to.equal(MavletEvent.ACTIVE_TRANSPORT_SET)
    expect(eventsStub.getCall(1).args[0], '2').to.equal(MavletEvent.ACTIVE_ACCOUNT_SET)
    expect(eventsStub.getCall(2).args[0], '3').to.equal(MavletEvent.ACTIVE_TRANSPORT_SET)
    expect(eventsStub.getCall(3).args[0], '4').to.equal(MavletEvent.CHANNEL_CLOSED)
  })
})
