...
import { MavletEvent, MavletEventHandlerFunction, MavletEventType } from '../events'
import {
  ConnectionContext,
  AccountInfo,
  TransportType,
...
import {
  ...
  isIOS,
  currentOS
} from '@mavrykdynamics/mavlet-ui'
import { WalletConnectTransport } from '@mavrykdynamics/mavlet-transport-walletconnect'
 ...
 * @publicapi
 *
 * The DAppClient has to be used in decentralized applications. It handles all the logic related to connecting to mavlet-compatible
...
export class DAppClient extends Client {
  ...
  protected readonly events: MavletEventHandler = new MavletEventHandler()
  ...
  private readonly openRequests = new Map<
    ...
    ExposedPromise<
      {
        message: MavletMessage | MavletMessageWrapper<MavletBaseMessage>
  ...
  private readonly bugReportStorage = new IndexedDBStorage('mavlet', 'bug_report')
  ...
  constructor(config: DAppClientOptions) {
    ...
    this.handleResponse = async (
      ...
      if (message.version === '3') {
        const typedMessage = message as MavletMessageWrapper<MavletBaseMessage>

        if (openRequest && typedMessage.message?.type === MavletMessageType.Acknowledge) {
          ...
          this.events
            .emit(MavletEvent.ACKNOWLEDGE_RECEIVED, {
          ...
          if (typedMessage.message?.type === MavletMessageType.PermissionResponse && appMetadata) {
            await this.appMetadataManager.addAppMetadata(appMetadata)
          }

          if (typedMessage.message?.type === MavletMessageType.Error) {
          ...
          if (typedMessage.message?.type === MavletMessageType.Disconnect) {
            ...
            if (relevantTransport) {
              ...
              await this.events.emit(MavletEvent.CHANNEL_CLOSED)
            }
          } else if (typedMessage.message?.type === MavletMessageType.ChangeAccountRequest) {
        ...
        const typedMessage = message as MavletMessage

        if (openRequest && typedMessage.type === MavletMessageType.Acknowledge) {
          logger.log('handleResponse', `acknowledge message received for ${message.id}`)
          this.analytics.track('event', 'DAppClient', 'Acknowledge received from Wallet')
          ...
          if (
            typedMessage.type === MavletMessageType.PermissionResponse &&
            typedMessage.appMetadata
          ) {
            await this.appMetadataManager.addAppMetadata(typedMessage.appMetadata)
          }

          if (typedMessage.type === MavletMessageType.Error || (message as any).errorType) {
          ...
          if (
            typedMessage.type === MavletMessageType.Disconnect ||
            (message as any)?.typedMessage?.type === MavletMessageType.Disconnect // TODO: TYPE
          ) {
            this.analytics.track('event', 'DAppClient', 'Disconnect received from Wallet')
          ...
          } else if (typedMessage.type === MavletMessageType.ChangeAccountRequest) {
  ...
  private async wcToastHandler(status: string) {
    ...
    await this.events.emit(MavletEvent.HIDE_UI, ['alert'])
    if (status === 'pending') {
      this.events.emit(MavletEvent.ACKNOWLEDGE_RECEIVED, {
        message: {} as any,
        extraInfo: {} as any,
        walletInfo
      })
    } else {
      this.events.emit(MavletEvent.PERMISSION_REQUEST_ERROR, {
        errorResponse: { errorType: MavletErrorType.ABORTED_ERROR } as any,
        walletInfo
      })
    }
  }
        ...
        if (activeAccount && activeAccount.origin) {
          ...
          postMessageTransport
            .listenForNewPeer((peer) => {
              ...
              this.events
                .emit(MavletEvent.PAIR_SUCCESS, peer)
          ...
          p2pTransport
            .listenForNewPeer((peer) => {
              ...
              this.analytics.track('event', 'DAppClient', 'Mavlet Wallet connected', {
                peerName: peer.name
              })
              this.events
                .emit(MavletEvent.PAIR_SUCCESS, peer)
  ...
  private async resetInvalidState(emit: boolean = true) {
    ...
    emit && this.events.emit(MavletEvent.INVALID_ACTIVE_ACCOUNT_STATE)
  ...
  public async setActiveAccount(account?: AccountInfo): Promise<void> {
    ...
    await this.events.emit(MavletEvent.ACTIVE_ACCOUNT_SET, account)
  ...
  public async showPrepare(): Promise<void> {
    ...
    await this.events.emit(MavletEvent.SHOW_PREPARE, { walletInfo })
  }

  public async hideUI(elements: ('alert' | 'toast')[]): Promise<void> {
    await this.events.emit(MavletEvent.HIDE_UI, elements)
  }
  ...
  private sendMetrics(
    ...
    fetch(`https://mavlet-backend.prod.gke.papers.tech/${uri}`, options)
   ...
   */
  public async subscribeToEvent<K extends MavletEvent>(
    internalEvent: K,
    eventCallback: MavletEventHandlerFunction<MavletEventType[K]>
  ): Promise<void> {
    if (internalEvent === MavletEvent.ACTIVE_ACCOUNT_SET) {
   ...
   */
  public async checkPermissions(type: MavletMessageType): Promise<boolean> {
    if (
      [
    ...
    switch (type) {
      ...
      // TODO: ENCRYPTION
      // case MavletMessageType.EncryptPayloadRequest:
  ...
  public async permissionRequest(
    ...
    >(request).catch(async (requestError: ErrorResponse) => {
      requestError.errorType === MavletErrorType.ABORTED_ERROR
  ...
  public async request(input: BlockchainRequestV3<string>): Promise<BlockchainResponseV3<string>> {
    ...
    const request: BlockchainRequestV3<string> = {
      ...input,
      type: MavletMessageType.BlockchainRequest,
    ...
    const { message: response, connectionInfo } = await this.makeRequestV3<
      ...
      MavletMessageWrapper<BlockchainResponseV3<string>>
    >(request).catch(async (requestError: ErrorResponse) => {
  ...
  public async requestPermissions(
    ...
    if (input?.network !== undefined && this.network.type !== input?.network?.type) {
      console.error(
        '[MAVLET] The network specified in the DAppClient constructor does not match the network set in the permission request. Please set the network in the constructor. Setting it during the Permission Request is deprecated.'
      )
    }
  ...
  //     type: MavletMessageType.EncryptPayloadRequest,
  ...
  public async requestBroadcast(input: RequestBroadcastInput): Promise<BroadcastResponseOutput> {
    ...
    if (input.network !== undefined && this.network.type !== input.network?.type) {
      console.error(
        '[MAVLET] The network specified in the DAppClient constructor does not match the network set in the broadcast request. Please set the network in the constructor. Setting it during the Broadcast Request is deprecated.'
      )
    }
  ...
  protected async setTransport(transport?: Transport<any>): Promise<void> {
    ...
    await this.events.emit(MavletEvent.ACTIVE_TRANSPORT_SET, event)
  ...
  private async sendInternalError(errorMessage: string): Promise<void> {
    await this.events.emit(MavletEvent.INTERNAL_ERROR, { text: errorMessage })
   ...
   * @param mavletError The error we received
   */
  private async handleRequestError(
    request: MavletRequestInputMessage,
    mavletError: ErrorResponse
  ): Promise<void> {
    logger.error('handleRequestError', 'error response', mavletError)
    if (mavletError.errorType) {
      const buttons: AlertButton[] = []
      if (mavletError.errorType === MavletErrorType.NO_PRIVATE_KEY_FOUND_ERROR) {
      ...
      if (
        request.type === MavletMessageType.PermissionRequest &&
      ...
      throw MavletError.getError(mavletError.errorType, mavletError.errorData)
    }
   ...
   *
   * @param requestInput The MavletMessage to be sent to the wallet
   * @param account The account that the message will be sent to
   * @param skipResponse If true, the function return as soon as the message is sent
   */

  private makeRequest<T extends MavletRequestInputMessage, U extends MavletMessage>(
  ...
  private async makeRequest<T extends MavletRequestInputMessage, U extends MavletMessage>(
    requestInput: Optional<T, IgnoredRequestInputProperties>,
    ...
    if (await this.addRequestAndCheckIfRateLimited()) {
      this.events
        .emit(MavletEvent.LOCAL_RATE_LIMIT_REACHED)
        .catch((emitError) => console.warn(emitError))
    ...
    if (!(await this.checkPermissions(requestInput.type))) {
      this.events.emit(MavletEvent.NO_PERMISSIONS).catch((emitError) => console.warn(emitError))

      throw new Error('No permissions to send this request to wallet!')
    }

    if (!this.mavletId) {
      throw await this.sendInternalError('MavletID not defined')
    }
    ...
    const request: Optional<T, IgnoredRequestInputProperties> &
      ...
      senderId: await getSenderId(await this.mavletId),
      ...requestInput
    }
    ...
    try {
      ...
      this.events.emit(MavletEvent.INTERNAL_ERROR, {
  ...
  private async makeRequestV3<
    ...
    U extends MavletMessageWrapper<BlockchainMessage<string>>
  >(
    requestInput: T
  ): Promise<{
    message: U
    ...
    const request: MavletMessageWrapper<BlockchainMessage> = {
      id: messageId,
      version: '3',
      senderId: await getSenderId(await this.mavletId),
    ...
    try {
      ;(await this.transport).send(payload, peer)
      if (
        request.message.type !== MavletMessageType.PermissionRequest ||
        (this._activeAccount.isResolved() && (await this._activeAccount.promise))
      ) {
        this.tryToAppSwitch()
      }
    ...
    const index = requestInput.type as any as MavletMessageType
