...
import { ExposedPromise, toHex } from '@mavrykdynamics/mavlet-utils'
 ...
 * @publicapi
 *
 * The WalletClient has to be used in the wallet. It handles all the logic related to connecting to mavlet-compatible
...
export class WalletClient extends Client {
  ...
  private pendingRequests: [
    MavletRequestMessage | MavletMessageWrapper<MavletBaseMessage>,
    ConnectionContext
  ][] = []
  ...
  public async connect(
    newMessageCallback: (
      message: MavletRequestOutputMessage,
      connectionContext: ConnectionContext
    ) => void
  ): Promise<void> {
    this.handleResponse = async (
      message: MavletRequestMessage | MavletMessageWrapper<MavletBaseMessage> | DisconnectMessage,
      connectionContext: ConnectionContext
    ): Promise<void> => {
      if (message.version === '3') {
        ...
        const typedMessage = message as MavletRequestMessage | DisconnectMessage

        if (typedMessage.type === MavletMessageType.Disconnect) {
          return this.disconnect(typedMessage.senderId)
        }
   ...
   *
   * @param message The MavletResponseMessage that will be sent back to the DApp
   */
  public async respond(message: MavletResponseInputMessage): Promise<void> {
    ...
    await OutgoingResponseInterceptor.intercept({
      ...
      appMetadataManager: this.appMetadataManager,
      interceptorCallback: async (response: MavletMessage): Promise<void> => {
  ...
  /**
   * An internal method to send a MavletMessage to the DApp
   *
   * @param response Send a message back to the DApp
   */
  private async respondToMessage(
    response: MavletMessage,
    connectionContext: ConnectionContext
  ): Promise<void> {
