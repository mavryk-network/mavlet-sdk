...
import { AppMetadataManager, Logger } from '@mavrykdynamics/mavlet-core'
import { SimulatedProofOfEventChallengeRequestOutput } from '@mavrykdynamics/mavlet-types/dist/esm/types/mavlet/messages/MavletRequestOutputMessage'

const logger = new Logger('IncomingRequestInterceptor')

interface IncomingRequestInterceptorOptions {
  message: MavletRequestMessage | MavletMessageWrapper<MavletBaseMessage>
  connectionInfo: ConnectionContext
  appMetadataManager: AppMetadataManager
  interceptorCallback(message: MavletRequestOutputMessage, connectionInfo: ConnectionContext): void
}

interface IncomingRequestInterceptorOptionsV2 extends IncomingRequestInterceptorOptions {
  message: MavletRequestMessage
}

interface IncomingRequestInterceptorOptionsV3 extends IncomingRequestInterceptorOptions {
  message: MavletMessageWrapper<MavletBaseMessage>
}
/**
 * @internalapi
 *
...
export class IncomingRequestInterceptor {
  ...
  private static async handleV2Message(config: IncomingRequestInterceptorOptionsV2) {
    ...
    switch (message.type) {
      case MavletMessageType.PermissionRequest:
        {
          ...
          if ((message.appMetadata as any).mavletId && !message.appMetadata.senderId) {
            message.appMetadata.senderId = (message.appMetadata as any).mavletId
            delete (message.appMetadata as any).mavletId
          }
      ...
      case MavletMessageType.BroadcastRequest:
        ...
        break
      case MavletMessageType.ProofOfEventChallengeRequest:
        {
          const appMetadata: AppMetadata = await IncomingRequestInterceptor.getAppMetadata(
            appMetadataManager,
            message.senderId
        ...
        break
      case MavletMessageType.SimulatedProofOfEventChallengeRequest:
        {
          const appMetadata: AppMetadata = await IncomingRequestInterceptor.getAppMetadata(
            appMetadataManager,
            message.senderId
  ...
  private static async handleV3Message(config: IncomingRequestInterceptorOptionsV3) {
    ...
    const wrappedMessage:
      | MavletMessageWrapper<PermissionRequestV3<string>>
      | MavletMessageWrapper<BlockchainRequestV3<string>> = msg as any /* TODO: Remove any */
    ...
    switch (v3Message.type) {
      case MavletMessageType.PermissionRequest:
        ...
        break
      case MavletMessageType.BlockchainRequest:
        {
          // const appMetadata: AppMetadata = await IncomingRequestInterceptor.getAppMetadata(
          //   appMetadataManager,
          //   msg.senderId
