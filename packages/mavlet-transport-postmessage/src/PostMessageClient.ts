import { windowRef, Serializer, getSenderId, MessageBasedClient } from '@mavrykdynamics/mavlet-core'

import { openCryptobox, secretbox_NONCEBYTES, secretbox_MACBYTES } from '@mavrykdynamics/mavlet-utils'
import {
  ExtensionMessage,
  ...
  public async listenForChannelOpening(
    ...
    const fn = async (event: any): Promise<void> => {
      if (event.source !== windowRef || event.origin !== windowRef.location.origin) {
        // TODO: Add to error handler: console.debug('[Mavlet]: Event received from untrusted origin')
        return
      }
