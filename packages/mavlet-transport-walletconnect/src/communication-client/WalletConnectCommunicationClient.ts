...
import { generateGUID, getAddressFromPublicKey } from '@mavrykdynamics/mavlet-utils'
...
export class WalletConnectCommunicationClient extends CommunicationClient {
  ...
  private abortErrorBuilder() {
    ...
    const errorResponse: any = {
      ...
      errorType: MavletErrorType.ABORTED_ERROR
    }
  ...
  private async notifyListeners(topic: string, partialResponse: MavletInputMessage) {
    const response: MavletBaseMessage = {
      ...partialResponse,
      version: '2',
      senderId: topic
    }
