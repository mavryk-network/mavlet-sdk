...
describe(`LocalStorage`, () => {
  ...
  it(`should write a value to the storage`, async () => {
    ...
    await storage.set(StorageKey.MAVLET_SDK_VERSION, testValue)

    expect(setSpy.firstCall.args[0]).to.deep.equal(StorageKey.MAVLET_SDK_VERSION)
  ...
  it(`should delete a value to the storage`, async () => {
    ...
    await storage.delete(StorageKey.MAVLET_SDK_VERSION)

    expect(deleteSpy.firstCall.args[0]).to.deep.equal(StorageKey.MAVLET_SDK_VERSION)
  ...
  it(`should read a default value from the storage`, async () => {
    ...
    const valueVersion = await storage.get(StorageKey.MAVLET_SDK_VERSION)

    expect(valueVersion).to.be.undefined
    expect(getSpy.firstCall.args[0]).to.equal(StorageKey.MAVLET_SDK_VERSION)
  ...
  it(`should read a value from the storage`, async () => {
    ...
    localStorage.setItem(StorageKey.MAVLET_SDK_VERSION, testValue)

    const value = await storage.get(StorageKey.MAVLET_SDK_VERSION)
  ...
  it(`should create a storage with a prefix`, async () => {
    ...
    localStorage.setItem(prefix1 + '-' + StorageKey.MAVLET_SDK_VERSION, testValue1)
    localStorage.setItem(prefix2 + '-' + StorageKey.MAVLET_SDK_VERSION, testValue2)
    localStorage.setItem(StorageKey.MAVLET_SDK_VERSION, testValueNoPrefix)

    const value1 = await storage1.get(StorageKey.MAVLET_SDK_VERSION)
    const value2 = await storage2.get(StorageKey.MAVLET_SDK_VERSION)
    const valueNoPrefix = await storageNoPrefix.get(StorageKey.MAVLET_SDK_VERSION)

    expect(value1).to.equal(testValue1)
    expect(value2).to.equal(testValue2)
    expect(valueNoPrefix).to.equal(testValueNoPrefix)
    expect(getSpy.firstCall.args[0]).to.equal(prefix1 + '-' + StorageKey.MAVLET_SDK_VERSION)
    expect(getSpy.secondCall.args[0]).to.equal(prefix2 + '-' + StorageKey.MAVLET_SDK_VERSION)
    expect(getSpy.thirdCall.args[0]).to.equal(StorageKey.MAVLET_SDK_VERSION)
