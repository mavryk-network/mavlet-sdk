...
import { MavletClient } from '../mavlet-client/MavletClient'
 ...
 */
export abstract class Client extends MavletClient {
  protected readonly accountManager: AccountManager

  protected handleResponse: (
    _event: MavletRequestMessage | MavletMessageWrapper<MavletBaseMessage>,
  ...
  constructor(config: ClientOptions) {
    ...
    this.handleResponse = (
      message: MavletBaseMessage | MavletMessageWrapper<MavletBaseMessage>,
      connectionInfo: ConnectionContext
    ): void => {
      throw new Error(
  ...
  protected async addListener(transport: Transport<any>): Promise<void> {
    // in mavlet we subscribe to the transport on client init only
