...
import { Storage, StorageKey } from '@mavrykdynamics/mavlet-types'
...
export const migrate = async (storage: Storage): Promise<void> => {
  const lastSdkVersion: string | undefined = await storage.get(StorageKey.MAVLET_SDK_VERSION)
  ...
  await storage.set(StorageKey.MAVLET_SDK_VERSION, SDK_VERSION)
}
