...
import { ExposedPromise, generateGUID } from '@mavrykdynamics/mavlet-utils'
...
export class P2PCommunicationClient extends CommunicationClient {
  ...
  > {
    ...
    const results: { time: number; server: string; region: Regions; result: MavletInfoResponse }[] =
      []
    ...
    keys.forEach((key) => {
      ...
      const doRequest = (isFinalRequest: boolean = true) => {
        ...
        Promise.race([this.getMavletInfo(server), timeoutPromise.promise]).then((res) => {
  ...
  public async getRelayServer(): Promise<{ server: string; timestamp: number }> {
    if (this.relayServer) {
      ...
      const info = await this.getMavletInfo(relayServer.server)
    ...
    if (node && node.length > 0) {
      const info = await this.getMavletInfo(node)
      this.relayServer.resolve({
        server: node,
        timestamp: info.timestamp,
        localTimestamp: new Date().getTime()
  ...
  public async getMavletInfo(server: string): Promise<MavletInfoResponse> {
    return axios
      .get<MavletInfoResponse>(`https://${server}/_synapse/client/mavlet/info`)
      .then((res) => ({
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      if (this.loginCounter <= (this.ENABLED_RELAY_SERVERS[this.selectedRegion] ?? []).length) {
        ...
        logger.error(
          'start',
          'Tried to log in to every known mavlet node, but no login was successful.'
        )

        throw new Error('Could not connect to any mavlet nodes. Try again later.')
      }
    }
  ...
  public async updateRelayServer(sender: string) {
    ...
    const manager = localStorage.getItem('mavlet:communication-peers-dapp')
