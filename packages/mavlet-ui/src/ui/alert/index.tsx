...
import { Logger, Serializer, windowRef } from '@mavrykdynamics/mavlet-core'
...
const closeAlert = (_: string): Promise<void> => {
  return new Promise(async (resolve) => {
    ...
    if (dispose && isOpen()) {
      ...
      setTimeout(() => {
        if (dispose) dispose()
        if (document.getElementById('mavlet-alert-wrapper'))
          (document.getElementById('mavlet-alert-wrapper') as HTMLElement).remove()
      }, 500)
    }
    resolve()
  })
...
const openAlert = async (config: AlertConfig): Promise<string> => {
  ...
  if (!isOpen()) {
    ...
    shadowRootEl.setAttribute('id', 'mavlet-alert-wrapper')
