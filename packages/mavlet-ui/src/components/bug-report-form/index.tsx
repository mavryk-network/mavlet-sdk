import { IndexedDBStorage, Logger, SDK_VERSION } from '@mavrykdynamics/mavlet-core'
...
const BugReportForm = (props: any) => {
  ...
  const db = new IndexedDBStorage('mavlet', 'bug_report')
  ...
  const indexDBToMetadata = async () => {
    ...
    const mavletResult: StorageObject = {}
    let keys: string[] = []
    let values: string[] = []
    ...
    try {
      ...
      return [mavletResult, wcResult]
    }

    if (keys.length && values.length && keys.length === values.length) {
      keys.forEach(
        (key, i) => ((key.includes('mavlet') ? mavletResult : wcResult)[key] = values[i])
      )
    }
  ...
  const handleSubmit = async (event: Event) => {
    ...
    const [mavletState, wcState] = await indexDBToMetadata()

    const request: BugReportRequest = {
      userId:
        mavletState[StorageKey.USER_ID] && mavletState[StorageKey.USER_ID].length
          ? mavletState[StorageKey.USER_ID]
          : getUserId(),
      title: title(),
      ...
      localStorage: JSON.stringify(mavletState),
    ...
    fetch('https://mavlet-backend.prod.gke.papers.tech/bug-report/save', options)
      .then((response) => {
        if (!response.ok) {
